(function () {
  ...
  async function loadTicket() {
    ...
    try {
      ...
      // 401 часто будет логин-страница или JSON
      if (!resp.ok) {
        const ct = resp.headers.get('content-type') || '';

        // JSON-ошибка от твоего Node
        if (ct.includes('application/json')) {
          const j = await resp.json().catch(() => ({}));
          return setUiStatus(`❌ ${j.message || j.error || `HTTP ${resp.status}`}`, false);
        }

        // HTML/текстовая ошибка (например loginpage или nginx 503)
        const t = await resp.text().catch(() => '');
        const snippet = t.slice(0, 400).replace(/\s+/g, ' ').trim();

        // отдельное сообщение для 401
        if (resp.status === 401) {
          console.warn('[ticket_parser] 401 html snippet:', t.slice(0, 600));
          return setUiStatus('❌ 401: нет доступа. Cookie неверная/просрочена или нет прав на тикет.', false);
        }

        console.warn('[ticket_parser] HTTP error snippet:', snippet);
